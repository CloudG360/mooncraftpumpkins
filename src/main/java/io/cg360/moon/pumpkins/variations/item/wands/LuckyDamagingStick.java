...
public class LuckyDamagingStick extends PumpkinLuckyRoll {
    ...
    protected void run(Player player, Location<World> loc, int luckmult){
        ...
        itemStack.offer(Keys.DISPLAY_NAME, Text.of(TextColors.RED, TextStyles.BOLD, "Wand of Damaging"));
        itemStack.offer(Keys.ITEM_LORE, Arrays.asList(
                    Text.of(TextColors.BLACK, String.format("cooldown:10,damaging:%s", lucklvl)),
                    Text.of(TextColors.GREEN, "What else is there to know?"),
                    Text.of(TextColors.GREEN, "You damage nearby players!"),
                    Text.of(""),
                    Text.of(TextColors.RED, TextStyles.BOLD, "Cooldown: 10s"),
                    Text.of(TextColors.GOLD, TextStyles.BOLD, String.format("Radius: %s blocks", lucklvl))
                ));
