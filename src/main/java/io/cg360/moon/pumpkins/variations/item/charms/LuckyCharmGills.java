...
public class LuckyCharmGills extends PumpkinLuckyRoll {

    @Override
    protected void run(Player player, Location<World> loc, int luckmult){
        ItemStack itemStack = ItemStack.builder().itemType(ItemTypes.PRISMARINE_SHARD).quantity(1).build();
        itemStack.offer(Keys.DISPLAY_NAME, Text.of(TextColors.DARK_AQUA, TextStyles.BOLD, "Gills Charm"));
        itemStack.offer(Keys.ITEM_LORE, Arrays.asList(
                    Text.of(TextColors.BLACK, "water_charm"),
                    Text.of(TextColors.DARK_AQUA, "Gives you Water Breathing"),
                    Text.of(TextColors.DARK_AQUA, "when held in the off hand.")
                ));
        ...
        player.sendMessage(Text.of(TextColors.GOLD, TextStyles.BOLD, "TREAT", TextStyles.RESET, TextColors.GOLD, " You got a Gills Charm!"));
    }
}
