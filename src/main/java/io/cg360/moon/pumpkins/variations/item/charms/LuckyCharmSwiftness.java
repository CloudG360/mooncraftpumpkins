...
public class LuckyCharmSwiftness extends PumpkinLuckyRoll {

    @Override
    protected void run(Player player, Location<World> loc, int luckmult){
        ItemStack itemStack = ItemStack.builder().itemType(ItemTypes.FEATHER).quantity(1).build();
        itemStack.offer(Keys.DISPLAY_NAME, Text.of(TextColors.DARK_AQUA, TextStyles.BOLD, "Swiftness Charm"));
        itemStack.offer(Keys.ITEM_LORE, Arrays.asList(
                    Text.of(TextColors.BLACK, "speed_charm"),
                    Text.of(TextColors.AQUA, "Gives you speed when held"),
                    Text.of(TextColors.AQUA, "in your off-hand.")
                ));
        ...
        player.sendMessage(Text.of(TextColors.GOLD, TextStyles.BOLD, "TREAT", TextStyles.RESET, TextColors.GOLD, " You got a Swiftness Charm!"));
    }
}
